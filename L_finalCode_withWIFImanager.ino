//=============================================================================
//=========== Main Application with Integrated WiFiManager ===================
//=============================================================================

// --- REQUIRED LIBRARIES ---
#include <WiFi.h>
#include <WiFiManager.h>         // WiFiManager: Added library
...
// --- PIN DEFINITIONS ---
// WiFiManager: Consolidated pin definitions here
#define TRIGGER_PIN 0            // Pin for the button to trigger the on-demand portal
const int ledPin = 2;            // Onboard LED
const int servoPin = 13;         // Servo motor pin

// WiFiManager: Removed hardcoded SSID and password
...
// --- HTML PAGE (No changes needed here) ---
...
void setup() {
    ...
    pinMode(TRIGGER_PIN, INPUT_PULLUP); // WiFiManager: Initialize the trigger pin
    ...
    WiFi.mode(WIFI_STA);
    WiFiManager wm;
    // wm.resetSettings(); // Uncomment for testing to wipe credentials on every boot
    wm.setConfigPortalTimeout(180); // Set auto-connect portal timeout to 3 minutes

    if (!wm.autoConnect("AutoConnectAP", "password")) {
      Serial.println("Failed to connect via auto-connect and hit timeout. Awaiting button press for on-demand portal.");
      // The code will continue to the loop(), where the LED will blink.
    } else {
      Serial.println("Connected to WiFi successfully!");
    }
    
    // --- Start services ONLY if WiFi is connected ---
    if (WiFi.status() == WL_CONNECTED) {
        digitalWrite(ledPin, HIGH); // Solid LED indicates connection
        Serial.print("IP Address: http://");
        Serial.println(WiFi.localIP());

        // Start mDNS
        if (!MDNS.begin("serialclicker")) {
        ...
        Serial.println("WiFi not connected. Server and mDNS not started.");
    }
}

void loop() {
    // WiFiManager: Check for on-demand portal trigger
    if (digitalRead(TRIGGER_PIN) == LOW) {
      Serial.println("Button pressed! Starting OnDemand Configuration Portal.");
      WiFiManager wm;
      wm.setConfigPortalTimeout(180); // Set on-demand portal timeout to 3 minutes

      if (wm.startConfigPortal("OnDemandAP", "password")) {
        // If the portal led to a successful connection
        Serial.println("Connected successfully from on-demand portal! Restarting ESP...");
        delay(1000);
        ESP.restart(); // Restart to ensure all services start correctly
      } else {
        Serial.println("Failed to connect and hit timeout from on-demand portal.");
      }
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      webSocket.loop(); // Only process websockets if connected
    } else {
      // If connection is lost, blink the LED
...
// --- WebSocket and Status Functions (No changes needed) ---
void broadcastStatus() {
    ...
    if(WiFi.status() == WL_CONNECTED) webSocket.broadcastTXT(jsonString); // Only broadcast if connected
